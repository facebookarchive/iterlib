...
dynamic_iterator dynamic::begin() const {
  return dynamic_iterator(this);
}

dynamic_iterator dynamic::end() const {
...
void dynamic::merge(const dynamic& other) {
  try {
    ...
    LOG(ERROR) << "Error merging: " << toJson()
               << " " << other.toJson() << " "
               << e.what();
    throw e;
  }
